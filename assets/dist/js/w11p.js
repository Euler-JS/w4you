...
//We use Artyom JS and others script to make this improovements on WebSites
//By Euler ----> Joao Alberto Jose Simango
/**
 * 
...
console.log("Tudo em um ", titulo, menu, rodape);
...
function loadAndSetLink(linkData) {
    console.log("Configurando Links ", linkData.innerText, " O Link : ", linkData.href);
    artyom.addCommands([
        {
            indexes: ["Ir para " + linkData.innerText],
            action: function (i) {
                console.log("Executar login");
                artyom.say("Indo para " + linkData.innerText);
                window.open("" + linkData.href, "_self");
            }
        }
    ]);
}

//Funcao para configurar Artyom para abrir links do Menu
Array.prototype.slice.call(document.getElementsByTagName("a")).forEach(element => {
    // console.log("Nome ", element.innerText, " Link ",element.href);
    loadAndSetLink(element)
});


//Carregar e congigurar accoes para comandos de Link


function loadMainArtyomScript() {
    loadArtyomSetups()
}


function loadArtyomSetups() {


    // let todosLinks = Array.prototype.slice.call(document.getElementsByTagName("a"));
    // console.log("Conteudo do Menu ", documentData)
    // console.log("Conteudo do Menu ", Array.prototype.slice.call(document.getElementsByTagName("button")))
    ...
    window.onload = function startContinuousArtyom() {
        artyom.initialize({
            ...
            artyom.say("Euler helper Inicializado com sucesso", artyom.getVoices());

            var currentTime = new Date();
            var hours = currentTime.getHours();
            if ((hours >= 0 && hours < 4) || (hours >= 18 && hours <= 23)) {
                artyom.say("Boa noite! Bem vido ao " + documentData.titulo);
            } else if (hours >= 4 && hours < 12) {
                artyom.say("Bom dia! Bem vido ao " + documentData.titulo);
            } else if (hours >= 12 && hours < 18) {
                artyom.say("Boa tarde! Bem vido ao " + documentData.titulo);
            }
        ...
        artyom.addCommands([
            ...
            {
                indexes: ["Pra esquerda", "Para esquerda"],
                action: function (i) {
                    console.log(document.body.offsetHeight, document.body.scrollHeight);
        ...
        artyom.addCommands([
            {
                indexes: ["Bom dia", "Boa tarde", "Boa noite", "Euler está aí", "está aí Euler", "você me ouve", "bom dia Euler", "Euler dom dia", "boa tarde Euler", "Euler boa tarde", "boa noite Euler", "Euler boa noite"],
                action: function (i) {
                    var currentTime = new Date();
                    var hours = currentTime.getHours();
                    if ((hours >= 0 && hours < 4) || (hours >= 18 && hours <= 23)) {
                        artyom.say("Em que posso ajudar?");
                    } else if (hours >= 4 && hours < 12) {
                        artyom.say("Em que posso ajudar?");
                    } else if (hours >= 12 && hours < 18) {
                        artyom.say("Em que posso ajudar?");
        ...
        // artyom.on(['Ler *'], true).then(function (i, wildcard) {
        //     if (wildcard == "menu") {
...
function lerMenu() {
    if (documentData.menu.trim() != "") {
        console.log("ALl Menu ", documentData.menu);
        artyom.say(documentData.menu,
            {
                onStart: function () {
                    setTimeout(function () {
        ...
        artyom.say("Menu Vazio");
    }
}

function lerRodape() {
    if (documentData.rodape.trim() != "") {
        artyom.say(documentData.rodape,
            {
                onStart: function () {
                    setTimeout(function () {
                        console.log("On Start reeding");
